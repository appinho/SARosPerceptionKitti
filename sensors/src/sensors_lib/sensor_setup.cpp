...
SensorSetup::SensorSetup(ros::NodeHandle nh, ros::NodeHandle pnh):
	stereo_vision_(nh, pnh),
	depth_completion_(nh, pnh),
	sensor_fusion_(nh, pnh),
	ground_extraction_(nh, pnh)	{

	sub_pointcloud_velo_ = nh.subscribe(
		"kitti/velo/pointcloud", 2, &SensorSetup::process, this);
	pub_pointcloud_velo_ = nh.advertise<PointCloud2>(
		"/sensors/velo/pointcloud/", 2);
}
...
void SensorSetup::process(
	const PointCloud2ConstPtr & msg_pointcloud_velo)
{
	// Filter velodyne pointcloud
	...
	for(const auto & point: pcl_velo->points){
 	...
 	PointCloud2 msg_pointcloud_filtered;
	toROSMsg(*pcl_velo_filtered, msg_pointcloud_filtered);
	msg_pointcloud_filtered.header.stamp.sec = msg_pointcloud_velo->header.stamp.sec;
	msg_pointcloud_filtered.header.stamp.nsec = msg_pointcloud_velo->header.stamp.nsec;
	msg_pointcloud_filtered.header.frame_id = msg_pointcloud_velo->header.frame_id;
	pub_pointcloud_velo_.publish(msg_pointcloud_filtered);
}
